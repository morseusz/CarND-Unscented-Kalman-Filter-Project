...
UKF::UKF() {
  ...
  int weights_length = 2*n_aug_ + 1;
  weights_ = VectorXd(weights_length);
  weights_(0) = lambda_ / (lambda_ + n_aug_);
  double remaining_weights = 0.5 / (n_aug_ + lambda_);
  for (int i = 1; i<weights_length; i++) {
    weights_(i) = remaining_weights;
  }
  ...
  if (!is_initialized_) {
    ...
    if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      */
      ...
      x_(0) = rho * cos(phi);    // px
      x_(1) = rho * sin(phi);    // py
      x_(2) = rhod * cos(phi);   // vx
      x_(3) = rhod * sin(phi);   // vy
...
void UKF::SigmaPointPrediction(double delta_t) {
  // Based on "Augmentation Assignment 2" and "Sigma Point Prediction Assignment 2"
...
void UKF::PredictMeanAndCovariance() {
  // Based on "Predicted Mean and Covariance Assignment 2"
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  /**
  Based on "Predict Radar Measurement Assignment 2" and UKF Update Assignment 2
  */
  int n_z = 3;
  MatrixXd Zsig = MatrixXd(n_z, 2 * n_aug_ + 1);
